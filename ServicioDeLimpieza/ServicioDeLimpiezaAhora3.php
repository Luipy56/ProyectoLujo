...
foreach ($habitaciones as $numeroHabitacion) {
    if (strpos($numeroHabitacion, '3') === 0) { // Filtrar por la centena 300
        ...
        $comandoPeticionHabitacion = "SELECT post_id FROM wp_postmeta WHERE meta_key = '_mphb_booking_price_breakdown' AND meta_value LIKE '%301 %';";
        $conexionPeticionHabitacion = mysqli_query($conexion, $comandoPeticionHabitacion);
	
        $post_id = null;
        $fechas = array();
	$respuestaPeticionHabitacion = array();
//Prueba
	...
	$comandoFechaOutMasCercana= "SELECT meta_value FROM wp_postmeta WHERE post_id = $post_id;";
//Prueba

		$fechaActual = date('Y-m-d');
        if ($post_id && $fechaActual >= $fechaInMasCercana && $fechaActual <= $fechas[1]) {
